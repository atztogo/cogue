...
def _plot_lattice_points(lattice, dim):
    ...
    mlab.points3d(lp[0], lp[1], lp[2], scale_factor=0.2, opacity=0.2, color=(0, 0, 0))
...
def _plot_atoms(cell, margin=1e-5, shift=[0, 0, 0], atom_scale=0.4):
    ...
    for x, y, z, s in zip(xs, ys, zs, symbols):
        ...
        mlab.points3d(
            x, y, z, resolution=16, scale_factor=covalent_radii[s], color=color
        )
