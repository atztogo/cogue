...
if __name__ == '__main__':
    ...
    parser.add_option("--r2h",
                      dest="is_r2h",
                      action="store_true",
                      help="Transform primitive Rhombohedral to hexagonal")
    parser.add_option("--t_mat",
                      dest="t_mat",
                      action="store",
                      type="string",                      
                      help="Multiply transformation matrix")
    ...
    if options.is_r2h:
        cell = rhomb2hex(cell)
        vasp.write_poscar(cell, "H" + args[0])

    if options.t_mat:
        t_mat = np.array([float(x) for x in options.t_mat.split()]).reshape(3, 3)
        cell = reduce_points(t_mat, cell)
    ...
    if options.s_mat:
        ...
        cell = atoms2cell(scell_phonopy)

    # prim_cell = get_primitive(cell)
    # vasp.write_poscar(prim_cell, "POSCAR-prim")
    write_cif_P1(cell, args[0] + ".cif")
    write_v_sim(cell, args[0] + ".ascii")
