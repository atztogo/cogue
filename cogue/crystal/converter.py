...
def reduce_points(tmat, cell, tolerance=1e-5):
    """Reduce overlapping atomic points."""
    ...
    return Cell(
        lattice=np.dot(cell.lattice, tmat),
...
def _bc2prim(cell):
    tmat = [[-0.5, 0.5, 0.5], [0.5, -0.5, 0.5], [0.5, 0.5, -0.5]]
    return reduce_points(tmat, cell)


def _cbc2prim(cell):
    tmat = [[0.5, 0.5, 0.0], [-0.5, 0.5, 0.0], [0.0, 0.0, 1.0]]
    return reduce_points(tmat, cell)


def _abc2prim(cell):
    tmat = [[0.0, 0.5, 0.5], [0.0, -0.5, 0.5], [1.0, 0.0, 0.0]]
    return reduce_points(tmat, cell)


def _bbc2prim(cell):
    tmat = [[0.5, 0.0, 0.5], [0.5, 0.0, -0.5], [0.0, 1.0, 0.0]]
    return reduce_points(tmat, cell)
...
def atoms2cell(phonopy_cell):
    """Convert PhonopyAtoms to cogue Cell."""
    return Cell(
        lattice=phonopy_cell.get_cell().T,
...
#########################
def cell2atoms(cell):
    """Conver cogue Cell to PhonopyAtoms."""
    try:
