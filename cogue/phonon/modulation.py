...
class PhononModulation:
    ...
    def get_modulation_cells(self):
        return [self._get_cell_with_modulation(m) for m in self.get_modulations()]

    def get_modulations(self):
        return [
            self._get_modulation(pt) / ph * a for (pt, ph, a) in self._points_on_sphere
        ]
    ...
    def _run(self):
        ...
        for point in self._get_phases():
            ...
            for phase in phase_shifts:
                ...
                modcell = self._get_cell_with_modulation(modulation / phase * amplitude)
        ...
        for i in range(self._modulation_dimension[0]):
            for j in range(self._modulation_dimension[1]):
                for k in range(self._modulation_dimension[2]):
                    phase = np.exp(2j * np.pi * np.dot([i, j, k], 1.0 / dim))
                    is_found = True
...
class PhononModulationOld:
    ...
    def _run(self):
        ...
        for i, point in enumerate(self._get_all_points_on_sphere()):
            modcell = self._get_cell_with_modulation(self._get_modulation(point))
            symmetry = get_symmetry_dataset(modcell, tolerance=self._symmetry_tolerance)
