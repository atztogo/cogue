...
def plot(results, connections, confluences, reference_energy, ig_tids):
    ...
    lines = []
    pos = collect_points_lines(
        points, lines, tid_pos, tids[0], 0, energies, texts, tids, connections, ig_tids
    )
...
parser = OptionParser()
parser.set_defaults(
    reference_energy=0.0, ignore_tids=None, output_filename=None, ymax=None, ymin=None
)
parser.add_option(
    "--reference",
    dest="reference_energy",
    type="float",
...
parser.add_option(
    ...
    action="store",
    type="string",
    help="Draw without the TIDs",
)
parser.add_option(
    "--ymax", dest="ymax", type="float", help="Maximum y value of plot area"
)
parser.add_option(
    "--ymin", dest="ymin", type="float", help="Minimum y value of plot area"
)
